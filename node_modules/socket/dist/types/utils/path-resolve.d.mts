import type { SocketYml } from '@socketsecurity/config';
import type { SocketSdkReturnType } from '@socketsecurity/sdk';
export declare function findBinPathDetailsSync(binName: string): {
    name: string;
    path: string | undefined;
    shadowed: boolean;
};
export declare function findNpmPathSync(npmBinPath: string): string | undefined;
export type PackageFilesForScanOptions = {
    cwd?: string | undefined;
    config?: SocketYml | undefined;
};
export declare function getPackageFilesForScan(inputPaths: string[], supportedFiles: SocketSdkReturnType<'getReportSupportedFiles'>['data'], options?: PackageFilesForScanOptions | undefined): Promise<string[]>;
//# sourceMappingURL=path-resolve.d.mts.map